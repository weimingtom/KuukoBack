...
namespace kurumi
{
	...
	public class LuaParser
	{
		...
		public static void luaY_checklimit(FuncState fs, int v, int l, CLib.CharPtr m) 
		{ 
			if ((v) > (l)) 
			{
				errorlimit(fs, l, m);
	    ...
	    */
		...
		private static void error_expected(LuaLex.LexState ls, int token) 
		{
			LuaLex.luaX_syntaxerror(ls,
				LuaObject.luaO_pushfstring(ls.L, CLib.CharPtr.toCharPtr(LuaConf.getLUA_QS() + " expected"), LuaLex.luaX_token2str(ls, token)));
		}

		private static void errorlimit(FuncState fs, int limit, CLib.CharPtr what) 
		{
			CLib.CharPtr msg = (fs.f.linedefined == 0) ?
				LuaObject.luaO_pushfstring(fs.L, CLib.CharPtr.toCharPtr("main function has more than %d %s"), limit, what) :
				LuaObject.luaO_pushfstring(fs.L, CLib.CharPtr.toCharPtr("function at line %d has more than %d %s"),
		...
		public static void check_condition(LuaLex.LexState ls, bool c, CLib.CharPtr msg)
		{
			if (!(c)) 
			{
			...
			if (testnext(ls, what) == 0) 
			{
				...
				else 
				{
					LuaLex.luaX_syntaxerror(ls, LuaObject.luaO_pushfstring(ls.L,
						CLib.CharPtr.toCharPtr(LuaConf.getLUA_QS() + " expected (to close " + LuaConf.getLUA_QS() + " at line %d)"),
		...
		private static int registerlocalvar (LuaLex.LexState ls, LuaObject.TString varname)
		{
			...
			LuaMem.luaM_growvector_LocVar(ls.L, /*ref*/ locvars_ref, fs.nlocvars,
			    /*ref*/ sizelocvars_ref, (int)CLib.SHRT_MAX, 
				CLib.CharPtr.toCharPtr("too many local variables"), new ClassType(ClassType.TYPE_LOCVAR));
		...
		public static void new_localvarliteral(LuaLex.LexState ls, CLib.CharPtr v, int n) 
		{
			new_localvar(ls, LuaLex.luaX_newstring(ls, CLib.CharPtr.toCharPtr("" + v), /*(uint)*/(v.chars.Length - 1)), n);
		}
		...
		private static int indexupvalue(FuncState fs, LuaObject.TString name, expdesc v) 
		{
			...
			LuaMem.luaM_growvector_TString(fs.L, /*ref*/ upvalues_ref, f.nups, /*ref*/ sizeupvalues_ref, LuaLimits.MAX_INT, CLib.CharPtr.toCharPtr(""), new ClassType(ClassType.TYPE_TSTRING));
		...
		private static void pushclosure(LuaLex.LexState ls, FuncState func, expdesc v) 
		{
			...
			LuaMem.luaM_growvector_Proto(ls.L, /*ref*/ p_ref, fs.np, /*ref*/ sizep_ref,
				LuaOpCodes.MAXARG_Bx, CLib.CharPtr.toCharPtr("constant table overflow"), new ClassType(ClassType.TYPE_PROTO));
			f.sizep = sizep_ref[0];
			f.p = p_ref[0];
			...
			{
				LuaOpCodes.OpCode o = ((int)func.upvalues[i].k == (int)expkind.VLOCAL) ? LuaOpCodes.OpCode.OP_MOVE : LuaOpCodes.OpCode.OP_GETUPVAL;
		...
		public static LuaObject.Proto luaY_parser(LuaState.lua_State L, LuaZIO.ZIO z, LuaZIO.Mbuffer buff, CLib.CharPtr name) 
		{
		...
		private static void constructor(LuaLex.LexState ls, expdesc t) 
		{
			...
			check_match(ls, '}', '{', line);
			lastlistfield(fs, cc);
			LuaOpCodes.SETARG_B(new LuaCode.InstructionPtr(fs.f.code, pc), LuaObject.luaO_int2fb((int/*uint*/)cc.na)); /* set initial array size */
			LuaOpCodes.SETARG_C(new LuaCode.InstructionPtr(fs.f.code, pc), LuaObject.luaO_int2fb((int/*uint*/)cc.nh));  /* set initial table size */
		}
				...
				case (int)LuaLex.RESERVED.TK_DOTS: 
					{  
						...
						check_condition(ls, fs.f.is_vararg != 0,
							CLib.CharPtr.toCharPtr("cannot use " + LuaConf.LUA_QL("...") + " outside a vararg function"));
		...
		private static void assignment(LuaLex.LexState ls, LHS_assign lh, int nvars) 
		{
            ...
            check_condition(ls, 
				...
				CLib.CharPtr.toCharPtr("syntax error"));
			if (testnext(ls, ',') != 0) 
			{  
				...
				luaY_checklimit(ls.fs, nvars, LuaConf.LUAI_MAXCCALLS - ls.L.nCcalls,
					CLib.CharPtr.toCharPtr("variables in assignment"));
		...
		private static void forbody(LuaLex.LexState ls, int base_, int line, int nvars, int isnum) 
		{
			...
			endfor = (isnum != 0) ? LuaCode.luaK_codeAsBx(fs, LuaOpCodes.OpCode.OP_FORLOOP, base_, LuaCode.NO_JUMP) :
				LuaCode.luaK_codeABC(fs, LuaOpCodes.OpCode.OP_TFORLOOP, base_, 0, nvars);
