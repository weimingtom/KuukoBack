...
namespace kurumi
{
	...
	public class LuaStrLib
	{
		...
		public static int str_sub(LuaState.lua_State L) 
		{
			...
			if (start <= end)
			{
				LuaAPI.lua_pushlstring(L, CLib.CharPtr.plus(s, start - 1), /*(uint)*/(end - start + 1));
			}
			else 
			{
		...
		private static CLib.CharPtr matchbalance(MatchState ms, CLib.CharPtr s,
			CLib.CharPtr p) 
		{
		...
		private static CLib.CharPtr max_expand(MatchState ms, CLib.CharPtr s,
			CLib.CharPtr p, CLib.CharPtr ep) 
		{
		...
		private static CLib.CharPtr min_expand(MatchState ms, CLib.CharPtr s,
			CLib.CharPtr p, CLib.CharPtr ep) 
		{
			for (;;)
			{
		...
		private static CLib.CharPtr start_capture(MatchState ms, CLib.CharPtr s,
			CLib.CharPtr p, int what) 
		{
			CLib.CharPtr res;
			int level = ms.level;
		...
		private static CLib.CharPtr end_capture(MatchState ms, CLib.CharPtr s,
			CLib.CharPtr p) 
		{
		...
		private static CLib.CharPtr match_capture(MatchState ms, CLib.CharPtr s, int l)
		{
			...
			if ((int/*uint*/)CLib.CharPtr.minus(ms.src_end, s) >= len &&
			    CLib.memcmp(ms.capture[l].init, s, len) == 0)
			{
							...
							switch (p.get(1)) 
							{
								...
								case 'f': 
									{  
										...
										if (p.get(0) != '[')
										{
											LuaAuxLib.luaL_error(ms.L, CLib.CharPtr.toCharPtr("missing " + LuaConf.LUA_QL("[") + " after " +
												LuaConf.LUA_QL("%%f") + " in pattern"));
										}
										ep = classend(ms, p);  /* points to what is next */
										previous = (CLib.CharPtr.isEqual(s, ms.src_init)) ? '\0' : s.get(-1);
										if ((matchbracketclass((byte)(previous), p, CLib.CharPtr.minus(ep, 1)) != 0) ||
										    (matchbracketclass((byte)(s.get(0)), p, CLib.CharPtr.minus(ep, 1)) == 0))
										{
											return null;
										}
										p = ep; 
		...
		private static CLib.CharPtr lmemfind(CLib.CharPtr s1, int/*uint*/ l1,
			CLib.CharPtr s2, int/*uint*/ l2) 
		{
			if (l2 == 0) 
			{
			...
			else 
			{
				...
				while (l1 > 0 && CLib.CharPtr.isNotEqual((init = CLib.memchr(s1, s2.get(0), l1)), null))
				{
					...
					{  
						...
						l1 -= (/*uint*/int)CLib.CharPtr.minus(init, s1);
						s1 = init;
					}
				}
		...
		private static void push_onecapture(MatchState ms, int i, CLib.CharPtr s,
			CLib.CharPtr e) 
		{
			if (i >= ms.level) 
			{
				...
				{
					LuaAPI.lua_pushlstring(ms.L, s, /*(uint)*/CLib.CharPtr.minus(e, s));  /* add whole match */
				}
				else
				{
		...
		private static int str_find_aux(LuaState.lua_State L, int find) 
		{
			...
			CLib.CharPtr s = LuaAuxLib.luaL_checklstring(L, 1, /*out*/ l1);
			CLib.CharPtr p = LuaAuxLib.luaL_checklstring(L, 2, /*out*/ l2);
			...
			if ((find != 0) && ((LuaAPI.lua_toboolean(L, 4) != 0) ||  /* explicit request? */
				CLib.CharPtr.isEqual(CLib.strpbrk(p, CLib.CharPtr.toCharPtr(SPECIALS)), null)))
			{  
				/* or no special characters? */
				/* do a plain search */
				CLib.CharPtr s2 = lmemfind(CLib.CharPtr.plus(s, init), (/*uint*/int)(l1[0] - init), p, (/*uint*/int)(l2[0]));
		...
		public static int gmatch_aux(LuaState.lua_State L) 
		{
			...
			CLib.CharPtr s = LuaAPI.lua_tolstring(L, Lua.lua_upvalueindex(1), /*out*/ ls);
			CLib.CharPtr p = Lua.lua_tostring(L, Lua.lua_upvalueindex(2));
			CLib.CharPtr src;
			ms.L = L;
			ms.src_init = s;
			ms.src_end = CLib.CharPtr.plus(s, ls[0]);
			for (src = CLib.CharPtr.plus(s, /*(uint)*/LuaAPI.lua_tointeger(L, Lua.lua_upvalueindex(3)));
                 CLib.CharPtr.lessEqual(src, ms.src_end);
			     src = src.next()) 
			{
				CLib.CharPtr e;
				ms.level = 0;
				...
				{
                    int/*Int32*//*lua_Integer*/ newstart = CLib.CharPtr.minus(e, s);
		...
		{
			return LuaAuxLib.luaL_error(L, CLib.CharPtr.toCharPtr(LuaConf.LUA_QL("string.gfind") + " was renamed to " +
				LuaConf.LUA_QL("string.gmatch")));
		}

		private static void add_s(MatchState ms, LuaAuxLib.luaL_Buffer b, CLib.CharPtr s,
			CLib.CharPtr e) 
		{
			int[]/*uint*/ l = new int[1];
			int i;
			CLib.CharPtr news = LuaAPI.lua_tolstring(ms.L, 3, /*out*/ l);
			for (i = 0; i < l[0]; i++)
			{
				if (news.get(i) != L_ESC)
				{
				...
				else 
				{
					...
					{
						LuaAuxLib.luaL_addlstring(b, s, /*(uint)*/CLib.CharPtr.minus(e, s));
					}
					else 
					{
		...
		private static void add_value(MatchState ms, LuaAuxLib.luaL_Buffer b, CLib.CharPtr s,
			CLib.CharPtr e) 
		{
			...
			{  
				/* nil or false? */
				Lua.lua_pop(L, 1);
				LuaAPI.lua_pushlstring(L, s, /*(uint)*/CLib.CharPtr.minus(e, s));  /* keep original text */
			}
		...
		public static int str_gsub(LuaState.lua_State L) 
		{
			...
			CLib.CharPtr src = LuaAuxLib.luaL_checklstring(L, 1, /*out*/ srcl);
			...
			LuaAuxLib.luaL_addlstring(b, src, /*(uint)*/CLib.CharPtr.minus(ms.src_end, src));
			LuaAuxLib.luaL_pushresult(b);
		...
		private static CLib.CharPtr scanformat(LuaState.lua_State L, CLib.CharPtr strfrmt, CLib.CharPtr form) 
		{
			...
			if ((int/*uint*/)(CLib.CharPtr.minus(p, strfrmt)) >= (FLAGS.Length + 1))
			{
		...
		{
			int/*uint*/ l = /*(uint)*/CLib.strlen(form);
		...
		public static int str_format(LuaState.lua_State L) 
		{
			...
			CLib.CharPtr strfrmt = LuaAuxLib.luaL_checklstring(L, arg, /*out*/ sfl);
				...
				{ 
					...
					switch (ch)
					{
						...
						case 'i':
							{
								addintlen(form);
								CLib.sprintf(buff, form, (long/*Int64*//*LUA_INTFRM_T*/)LuaAuxLib.luaL_checknumber(L, arg));
								break;
							}
						case 'o':
						case 'u':
						...
						case 'X':
							{
								addintlen(form);
								CLib.sprintf(buff, form, (long/*UInt64*//*UNSIGNED_LUA_INTFRM_T*/)LuaAuxLib.luaL_checknumber(L, arg));
								break;
							}
						case 'e':
						case 'E':
						...
						default:
							{  /* also treat cases `pnLlh' */
								return LuaAuxLib.luaL_error(L, CLib.CharPtr.toCharPtr("invalid option " + LuaConf.LUA_QL("%%%c") + " to " +
									LuaConf.LUA_QL("format")), strfrmt.get(-1));
							}
					}
					LuaAuxLib.luaL_addlstring(b, buff, /*(uint)*/CLib.strlen(buff));
				}
			}
			LuaAuxLib.luaL_pushresult(b);
			return 1;
