...
public class LuaParser {
    ...
    */
	...
	public static void check_condition(LuaLex.LexState ls, boolean c, CLib.CharPtr msg) {
		if (!(c)) {
	...
	private static int registerlocalvar(LuaLex.LexState ls, LuaObject.TString varname) {
		...
		LuaMem.luaM_growvector_LocVar(ls.L, locvars_ref, fs.nlocvars, sizelocvars_ref, (int)CLib.SHRT_MAX, CLib.CharPtr.toCharPtr("too many local variables"), new ClassType(ClassType.TYPE_LOCVAR)); //ref - ref
	...
	public static void new_localvarliteral(LuaLex.LexState ls, CLib.CharPtr v, int n) {
		new_localvar(ls, LuaLex.luaX_newstring(ls, CLib.CharPtr.toCharPtr("" + v), (v.chars.length - 1)), n); //(uint)
	}
	...
	private static int indexupvalue(FuncState fs, LuaObject.TString name, expdesc v) {
		...
		LuaMem.luaM_growvector_TString(fs.L, upvalues_ref, f.nups, sizeupvalues_ref, LuaLimits.MAX_INT, CLib.CharPtr.toCharPtr(""), new ClassType(ClassType.TYPE_TSTRING)); //ref - ref
	...
	private static void pushclosure(LuaLex.LexState ls, FuncState func, expdesc v) {
		...
		sizep_ref[0] = f.sizep;
		LuaMem.luaM_growvector_Proto(ls.L, p_ref, fs.np, sizep_ref, LuaOpCodes.MAXARG_Bx, CLib.CharPtr.toCharPtr("constant table overflow"), new ClassType(ClassType.TYPE_PROTO)); //ref - ref
		f.sizep = sizep_ref[0];
		f.p = p_ref[0];
