...
//		 
	private static int luaB_print(lua_State L) {
		...
		for (i = 1; i <= n; i++) {
			...
			if (LuaConf.CharPtr.isEqual(s, null)) {
				return LuaAuxLib.luaL_error(L, LuaConf.CharPtr.toCharPtr(LuaConf.LUA_QL("tostring") + " must return a string to " + LuaConf.LUA_QL("print"))); //FIXME:
			}
			if (i > 1) {
	...
	private static void getfunc(lua_State L, int opt) {
		if (Lua.lua_isfunction(L, 1)) {
			...
			if (Lua.lua_isnil(L, -1)) {
				LuaAuxLib.luaL_error(L, LuaConf.CharPtr.toCharPtr("no function environment for tail call at level %d"), level);
			}
		}
	}
	...
	public static final LuaConf.CharPtr[] opts = { 
